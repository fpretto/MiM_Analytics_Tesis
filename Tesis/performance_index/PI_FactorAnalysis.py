...
class PI_FactorAnalysis:
    ...
    def get_factors(self, x, factors, method='principal', rotation='varimax', thresh_c_alpha=0.5):
        ...
        df_c_alpha = self.get_cronbach_alpha(x, df_factors_var, factors, threshold=thresh_c_alpha)

        return df_factors_var, df_c_alpha

    def get_cronbach_alpha(self, df, df_factors, factors, threshold=0.5):
        list_c_alpha = []
    ...
    def get_screeplot(self, df, cols):
        x = df[cols]
        fa = FactorAnalyzer()
        fa.fit(x, 10)
    ...
    def create_index(self, df, cols, dict_weights, position, factors, factors_method='principal', factors_rotation='varimax'):

        from sklearn.preprocessing import MinMaxScaler

        df_factors = self.get_factors(df[cols], factors=factors, method=factors_method, rotation=factors_rotation)[0]
        df_cronbach = self.get_factors(df[cols], factors=factors, method=factors_method, rotation=factors_rotation)[1]
        ...
        dict_scalers_position = {}
        # Scale performance index to a range of [0,1]
        scaler = MinMaxScaler()
        ...
        dict_scalers_position["index_scaler"] = scaler

        # Scale individual factors
        for id_factor in dict_weights_position.keys():
            if id_factor != 'Weights':
                scaler = MinMaxScaler()
                scaler = scaler.fit(pd.DataFrame(df_perf_idx[id_factor]))
                df_perf_idx[id_factor+"_scaled"] = scaler.transform(pd.DataFrame(df_perf_idx[id_factor]))
                dict_scalers_position[id_factor+"_scaler"] = scaler

        dict_weights_position["scalers"] = dict_scalers_position
        dict_weights[position] = dict_weights_position

        return df_perf_idx, dict_weights

    def score_index(self, df, dict_perf_index):
        ...
        for position in ['F', 'M', 'D', 'G']:
            ...
            # Scale Performance Index to a range of [0,1]
            df_index['Perf_Index_scaled'] = dict_perf_index['index_weights'][position]['scalers']['index_scaler'].transform(pd.DataFrame(df_index['Perf_Index']))
            ...
            for id_factor in dict_perf_index['index_weights'][position].keys():
                if (id_factor != 'Weights') & (id_factor != 'scalers'):
                    factors += 1
                    df_index[id_factor+"_scaled"] = dict_perf_index['index_weights'][position]['scalers'][id_factor+"_scaler"].transform(pd.DataFrame(df_index[id_factor]))

            # Append results
            cols_factors = ['Perf_Index_scaled'] + ['Factor {}_scaled'.format(fact+1) for fact in range(factors)]
            df_indexes = df_indexes.append(pd.concat([df[df['player_preferred_position'] == position].reset_index(drop=True),
                                                      df_index[cols_factors]], axis=1)).reset_index(drop=True)

        return df_indexes

    def sensitivity_analysis(self, df, dict_perf_index, path):
