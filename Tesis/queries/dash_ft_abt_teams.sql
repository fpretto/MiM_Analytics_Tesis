...
grouped_players_number AS(
	...
	ORDER BY player_id, team_id, matches DESC
),
grouped_players_number2 AS(
SELECT
	*,
	ROW_NUMBER() OVER(PARTITION BY player_id, team_id) AS row_n
...
LEFT JOIN fdm.dash_lk_player_position AS player_positions
	ON stats_players.player_id=player_positions.player_id AND stats_players.team_id=player_positions.team_id
LEFT JOIN fdm.dash_lk_player_number AS player_numbers
	ON stats_players.player_id=player_numbers.player_id AND stats_players.team_id=player_positions.team_id
...
GROUP BY teams.team_country_code, teams.team_country, matches.league_id,leagues.league_name, leagues.league_type,
...
SELECT * FROM fdm.dash_ft_abt_season_team