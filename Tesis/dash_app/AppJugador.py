...
# Tables
player = 'Carlos Tevez'
df_initial_player = df_season_players[df_season_players['player_name'] == player]
h2_player_title = f'{player} Statistics'
...
def set_season_options(selected_country):
    return [{'label': i, 'value': i} for i in dropdown_options['{}'.format(selected_country)].keys()]
...
def set_team_options(selected_season, selected_country):
    return [{'label': i, 'value': i} for i in dropdown_options['{}'.format(selected_country)]['{}'.format(selected_season)].keys()]
...
def set_players_options(selected_team, selected_season, selected_country):
    return [{'label': i, 'value': i} for i in dropdown_options['{}'.format(selected_country)]['{}'.format(selected_season)]['{}'.format(selected_team)]]
...
def update_player_data(player_dropdown_value):
    ...
    if player_dropdown_value != None:
        df_player = df_season_players.loc[(df_season_players['player_name'] == player_dropdown_value), :]

    # Tables
    df_season = create_player_table(df_player, phase='stats_season', season=config_data['Parameters']['current_season'])
    df_attack = create_player_table(df_player, phase='attack', season=config_data['Parameters']['current_season'])
    df_buildup = create_player_table(df_player, phase='build_up', season=config_data['Parameters']['current_season'])
    df_defense = create_player_table(df_player, phase='defense', season=config_data['Parameters']['current_season'])
    ...
    h2_player_title = f'{player_dropdown_value} Statistics'
