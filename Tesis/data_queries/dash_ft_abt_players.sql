-- Player Avg Team performance played
DROP TABLE IF EXISTS fdm.dash_lk_player_standings;
CREATE TABLE fdm.dash_lk_player_standings AS (
SELECT
	player_id,
	SUM(avg_team_position*appearances)/SUM(appearances) AS avg_team_position,
	SUM(avg_league_cov*appearances)/SUM(appearances) AS avg_league_cov

FROM fdm.dash_ft_abt_season_player
GROUP BY player_id);
...
SELECT
	...
	teams.team_name,
	teams.team_country,
	AVG(player_standings.avg_team_position) AS avg_team_position,
	AVG(player_standings.avg_league_cov) AS avg_league_cov,
...
LEFT JOIN fdm.dash_lk_player_team AS player_team
	ON stats_players.player_id=player_team.player_id
LEFT JOIN fdm.dash_lk_player_standings AS player_standings
	ON stats_players.player_id=player_standings.player_id
...
GROUP BY stats_players.player_id, player_names.player_name, player_positions.player_position, 
	player_numbers.player_number, player_team.team_id, teams.team_name, teams.team_country
	);

DROP TABLE fdm.dash_lk_player_position;
DROP TABLE fdm.dash_lk_player_number;
DROP TABLE fdm.dash_lk_player_team;
DROP TABLE fdm.dash_lk_player_standings;
